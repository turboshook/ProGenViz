...
func generate(parameters: Dictionary) -> void:
	...
	var rooms: Array[Rect2i] = [_get_random_rect(parameters)]
	
	# Place rooms until we hit the room limit or have retried too many times
	var retries: int = 0
	while !(rooms.size() >= parameters.max_room_count or retries >= parameters.retry_threshold):
